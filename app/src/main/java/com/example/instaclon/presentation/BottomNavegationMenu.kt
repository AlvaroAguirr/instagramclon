package com.example.instaclon.presentation
...
enum class  BottomNavegationitem(val icon: ImageVector, val route: Screens){
...
fun BottomNavigationMenu(
    selectItem: BottomNavegationitem,navController: NavController
){
    Row(modifier= Modifier.fillMaxWidth().wrapContentHeight().background(Color.White)){
        for(item in BottomNavegationitem.values()){
            Image(
                ...
                colorFilter = if(item==selectItem) ColorFilter.tint(Color.Black)
